...
for name in $leaf_directories; do
  rm -f $name/parser_error.log $name/interpreter_error.log $name/actual_end_state.okks
  ...
  if [ $parser_status != 0 ]; then
    ...
    ./../okk/interpreter ./../rules.okk $name/start_state.okks > $name/actual_end_state.okks 2> $name/interpreter_error.log
    interpreter_status=$?
